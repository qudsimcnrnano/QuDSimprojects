...
#include</opt/local/dune_210/dune-geometry-2.10.dev20221009/dune/geometry/referenceelement.hh>
...
void WaveFunction<GV,Vector>::find_maximum()
  {
    ...
    for(VertexIterator vt=gv.template begin<dim>();vt!=vtend;++vt)
      {
	if(fabs(wavevector[vertexmap.map(*vt)])>max)
	  {
	    max=fabs(wavevector[vertexmap.map(*vt)]);
	    max_coords=vt->geometry().corner(0);
	    //	    max_y=vt->geometry().corner(0)[1];
	    // cout<<"wavefunction maximum "<<fabs(wavevector[vertexmap.map(*vt)])<<" @  "<<max_coords<<endl;
  ...
  for(VertexIterator vt=gv.template begin<dim>();vt!=vtend;++vt)
      ...
      if(fabs(vt->geometry().corner(0)[0]-max_coords[0])<1e-8)
	{
	  if((fabs(vt->geometry().corner(0)[1])!=0)&&(fabs(vt->geometry().corner(0)[1])!=100))
	    {
	      if(wavevector[vertexmap.map(*vt)]<0)
		sign=-1;
	      if(wavevector[vertexmap.map(*vt)]>0)
		sign=1;
	      sign_tmp=sign;
  ...
  for(VertexIterator vt=gv.template begin<dim>();vt!=vtend;++vt)
      ...
      for(VertexIterator vjt=gv.template begin<dim>();vjt!=vtend;++vjt)
	{
	  if(fabs(vjt->geometry().corner(0)[1]-max_coords[1])<1e-8)
		...
		{
		  wavevector[vertexmap.map(*vt)]=wavevector[vertexmap.map(*vjt)];
		}
	    }
	}
    }
...
void WaveFunction<GV,Vector>::write_wf_gnuplot(const char* filename)
{
  ...
  for(VertexIterator vt=gv.template begin<dim>();vt!=vtend;++vt)
    {
      wf_plot<<vt->geometry().corner(0)[0]<<"    "<<vt->geometry().corner(0)[1]<<"    "<<vt->geometry().corner(0)[2]<<"    "<<wavevector[vertexmap.map(*vt)]<<std::endl;
    }
  wf_plot.close();
}
