// =============================================================================
// Parallel MPI Poisson FEM Solver with Newton-Raphson for Flash Memory (High-K)
// Key changes from serial poissonfem_NR.hh:
//   1. MPI rank-aware (file I/O on rank 0 only)
//   2. Per-element permittivity (epsi) support for multi-dielectric stacks
//   3. DUNE 2.10 API: ReferenceElements, mcmgMapper, leafGridView()
//   4. Boundary conditions adapted for flash memory geometry
// =============================================================================

#ifndef POISSONFEM_HIGHK_PARALLEL_HH
#define POISSONFEM_HIGHK_PARALLEL_HH
...
class PoissonFEM_HighK_Parallel
{
  private:
    ...
    Vector& epsi;          // per-element permittivity
    int it0, N;
    int rank, nprocs;
  ...
  public:
    PoissonFEM_HighK_Parallel(const GV& gv_, Vector& charge_, Vector& potential_,
                              double& Vss_, Vector& deltaRho_, Vector& deltapotential_,
                              int& it0_, Vector& epsi_);
    void apply();
};
...
// =============================================================================
template<class GV, class Vector>
PoissonFEM_HighK_Parallel<GV,Vector>::PoissonFEM_HighK_Parallel(
    const GV& gv_, Vector& charge_, Vector& potential_, double& Vss_,
    Vector& deltaRho_, Vector& deltapotential_, int& it0_, Vector& epsi_)
  : gv(gv_), charge(charge_), potential(potential_), set(gv.indexSet()),
    Vss(Vss_), deltaRho(deltaRho_), deltapotential(deltapotential_),
    it0(it0_), epsi(epsi_)
{
  ...
  N = gv.size(dim);
  if (rank == 0)
    std::cout << "  [PoissonFEM_HighK] dim = " << dim << ", N_local = " << N << std::endl;
  ...
  b.resize(N, false);
  R.resize(N, false);
  deltaRho_b.resize(N, false);
  AV.resize(N, false);
  u.resize(N, false);
  vertex_share.resize(N, false);
  cnt.resize(N, false);
  vertex_xcoord.resize(N, false);
  ...
  // Assemble stiffness matrix A with per-element permittivity
  ...
  for (LeafIterator it = gv.template begin<0>(); it != gv.template end<0>(); ++it) {
    ...
    // Get per-element permittivity
    double eps_elem = epsi[elementmap.index(*it)];
      ...
      for (int i = 0; i < vertexsize; i++) {
        ...
        for (int j = 0; j < vertexsize; j++) {
          ...
          A[set.subIndex(*it, i, dim)][set.subIndex(*it, j, dim)]
              += eps_elem * (grad1 * grad2) * weight * detjac;
        }
      }
    }
  }
...
// =============================================================================
template<class GV, class Vector>
void PoissonFEM_HighK_Parallel<GV,Vector>::determineAdjacencyPattern()
{
  adjacencyPattern.resize(N);
  if (rank == 0)
    std::cout << "  [PoissonFEM_HighK] Number of local unknowns: " << N << std::endl;
...
// =============================================================================
template<class GV, class Vector>
void PoissonFEM_HighK_Parallel<GV,Vector>::assemble()
{
  b = 0.0;
  NR = 0.0;
  deltaRho_b = 0.0;
...
// Gate at x=0 gets Vss, substrate contact at x=1 gets 0
// =============================================================================
template<class GV, class Vector>
void PoissonFEM_HighK_Parallel<GV,Vector>::apply_bc()
{
    ...
    for (IntersectionIterator is = gv.ibegin(*it); is != isend; ++is) {
      ...
      if (is->boundary()) {
        for (int i = 0; i < ref.size(is->indexInInside(), 1, dim); i++) {
          ...
          // Gate boundary (x==0) and substrate boundary (x==1)
...
// =============================================================================
template<class GV, class Vector>
void PoissonFEM_HighK_Parallel<GV,Vector>::solve()
{
  if (rank == 0)
    std::cout << "  [PoissonFEM_HighK] Solving (initial iteration)..." << std::endl;
  ...
  potential.resize(b.N(), false);
  potential = 1.0;
  bcgs.apply(potential, b, r);

  if (rank == 0)
    std::cout << "  [PoissonFEM_HighK] Initial solve converged: " << r.converged
...
// =============================================================================
template<class GV, class Vector>
void PoissonFEM_HighK_Parallel<GV,Vector>::solve_NR()
{
  if (rank == 0)
    std::cout << "  [PoissonFEM_HighK] Solving Newton-Raphson..." << std::endl;

  Dune::MatrixAdapter<Matrix, ScalarField, ScalarField> op_NR(NR);
  Dune::SeqILUn<Matrix, ScalarField, ScalarField> ilu1_NR(NR, 1, 0.1);
  Dune::BiCGSTABSolver<ScalarField> bcgs_NR(op_NR, ilu1_NR, 1e-100, 10000, (rank == 0) ? 1 : 0);
  Dune::InverseOperatorResult r;

  deltapotential.resize(R.N(), false);
  deltapotential = 1.0;
  bcgs_NR.apply(deltapotential, R, r);

  if (rank == 0)
    std::cout << "  [PoissonFEM_HighK] NR solve converged: " << r.converged
              << " in " << r.iterations << " iterations." << std::endl;
}
...
// =============================================================================
template<class GV, class Vector>
void PoissonFEM_HighK_Parallel<GV,Vector>::apply()
{
  assemble();
  apply_bc();
  A.mv(potential, AV);
...
#endif // POISSONFEM_HIGHK_PARALLEL_HH
